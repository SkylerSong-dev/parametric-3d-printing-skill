...
def test_strict_watertight(good_script):
    ...
    assert data["winding_consistent"] is True
    assert data["valid_volume"] is True
    assert data["mesh_reports"][0]["components"] == 1
...
def test_json_shape_keys(good_script):
    ...
    expected = {
        ...
        "winding_consistent", "valid_volume", "mesh_reports",
